...
class TestComplianceMechanic:
    """The compliance mechanic is opt-in via state["agent_plan"].

    A controller that never sets agent_plan keys must never be penalised
    (no double-jeopardy). A controller that DOES opt in (typically by
    extracting a directive from a qualitative_alert via an LLM) is
    penalised in proportion to how badly it breaches its own declared
    bounds.
    """

    @pytest.fixture
    def state_with_battery(self) -> dict:
        return {
            "time": 5,
            "demand": 50.0,
            "solar": 10.0,
    ...
    def test_no_penalty_when_agent_plan_absent(self, engine, state_with_battery):
        """No agent_plan = no compliance enforcement, regardless of SOC/exports."""
        _, outputs = engine.step(state_with_battery, do_nothing_action())
        assert outputs["cost_breakdown"]["compliance_penalty"] == pytest.approx(0.0)

    def test_no_penalty_when_compliance_window_absent(self, engine, state_with_battery):
        """min_soc_floor on its own (no window) is ignored — partial config = off."""
        state = {**state_with_battery, "agent_plan": {"min_soc_floor": 0.8}}
        _, outputs = engine.step(state, do_nothing_action())
        assert outputs["cost_breakdown"]["compliance_penalty"] == pytest.approx(0.0)

    def test_no_penalty_outside_compliance_window(self, engine, state_with_battery):
        """Window [10, 20] doesn't apply at time=5."""
        state = {
            **state_with_battery,
            "agent_plan": {
                "compliance_window": [10, 20],
                "min_soc_floor": 0.8,
            },
        }
        _, outputs = engine.step(state, do_nothing_action())
    ...
    def test_soc_floor_breach_charged(self, engine, state_with_battery):
        """SOC ends at 0.30 (no battery action), floor is 0.50 → shortfall 0.20."""
        state = {
            **state_with_battery,
            "agent_plan": {
    ...
    def test_soc_floor_satisfied_zero_penalty(self, engine, state_with_battery):
        """SOC well above floor → no penalty."""
        plenty = {**state_with_battery, "soc": 0.90}
        state = {
            **plenty,
            "agent_plan": {
                "compliance_window": [0, 10],
    ...
    def test_export_cap_breach_charged(self, engine):
        """Exporting 40 kW against a 10 kW cap → 30 kW excess × dt × rate."""
        # Lots of solar, no demand, full battery → max export
        state = {
            "time": 0,
            "demand": 5.0,
            "solar": 60.0,
        ...
        _, outputs = engine.step(state, do_nothing_action())
        # net_grid_power should be -55 (= 5 demand - 60 solar), capped at -50
        assert outputs["net_grid_power"] == pytest.approx(-50.0)
        # Exported 50 kW, cap 10, excess 40 kW
        cfg = engine.config
        expected = 40.0 * cfg.dt_hours * cfg.compliance_export_penalty_per_kw
        assert outputs["cost_breakdown"]["compliance_penalty"] == pytest.approx(expected)

    def test_export_cap_when_importing_no_penalty(self, engine, state_with_battery):
        """Importing power doesn't trip the export ceiling."""
        state = {
            **state_with_battery,
            "agent_plan": {
                "compliance_window": [0, 10],
        ...
        _, outputs = engine.step(state, do_nothing_action())
        # net_grid_power is +40 (importing), not exporting
        assert outputs["net_grid_power"] > 0
        assert outputs["cost_breakdown"]["compliance_penalty"] == pytest.approx(0.0)

    def test_both_constraints_sum(self, engine):
        """SOC shortfall AND export excess add up on the same line."""
        state = {
            "time": 0,
            "demand": 5.0,
            "solar": 60.0,
        ...
        _, outputs = engine.step(state, do_nothing_action())
        cfg = engine.config
        # SOC: end of step is 0.10 (idle), floor 0.40 → shortfall 0.30
        soc_part = 0.30 * cfg.compliance_soc_penalty_per_unit
        # Export: 40 kW excess × dt × rate
    ...
    def test_penalty_calibrated_below_blackout(self):
        """Max-soc compliance breach must be cheaper than a 1 kWh blackout
        — otherwise rational controllers shed load to satisfy a directive,
        which would be catastrophic in playtest."""
        from watt_the_hack.engine.engine import SimulationConfig

        cfg = SimulationConfig()
        # Worst-case per-step SOC penalty: full SOC unit shortfall
        worst_soc_penalty = 1.0 * cfg.compliance_soc_penalty_per_unit
        # 1 kWh of unmet demand at one step
        blackout_1kwh = 1.0 * cfg.blackout_penalty_per_kwh
        assert worst_soc_penalty < blackout_1kwh, (
            "compliance_soc_penalty_per_unit too aggressive — controllers "
            "may rationally blackout to comply"
        )

    def test_penalty_calibrated_above_compliance_cost(self):
        """A controller that complies pays the *one-off* wear of charging
        up; one that ignores eats a per-step penalty for the whole window.

        For the mechanic to bite, holding a 0.10-SOC breach for a typical
        4-step (1-hour) window must cost more than the wear to fix it
        once. Otherwise ignoring directives is strictly cheaper.
        """
        from watt_the_hack.engine.engine import SimulationConfig

        cfg = SimulationConfig()
        # Cost to comply: charge 0.10 SOC = 10 kWh through the battery
        # (we only count the wear, not the import cost — the import cost
        # is symmetric with not complying, so cancels for this calibration).
        wear_to_comply = 10.0 * cfg.battery_wear_cost_per_kwh
        # Cost of ignoring: pay the penalty per step for 4 steps (1h)
        ignore_4_steps = 0.10 * cfg.compliance_soc_penalty_per_unit * 4
        assert ignore_4_steps > wear_to_comply, (
            f"compliance penalty too soft — ignoring 1h of directive "
            f"({ignore_4_steps:.2f}) cheaper than complying "
            f"({wear_to_comply:.2f})"
        )

    def test_agent_plan_only_one_partial_key_safe(self, engine, state_with_battery):
        """An incomplete agent_plan (window but no constraint keys) is harmless."""
        state = {
            **state_with_battery,
            "agent_plan": {"compliance_window": [0, 10]},
        }
        _, outputs = engine.step(state, do_nothing_action())
        assert outputs["cost_breakdown"]["compliance_penalty"] == pytest.approx(0.0)

    def test_malformed_window_silently_disabled(self, engine, state_with_battery):
        """A non-list/tuple window is ignored — never crash on bad LLM output."""
        for bad in ("not a list", 5, None, [1, 2, 3], [1]):
            state = {
                **state_with_battery,
                "agent_plan": {
                    "compliance_window": bad,
                    "min_soc_floor": 0.9,
                },
            }
            _, outputs = engine.step(state, do_nothing_action())
    ...
    def test_malformed_constraint_values_silently_disabled(
        self, engine, state_with_battery
    ):
        """Non-numeric constraint values are ignored, not crashes."""
        state = {
            **state_with_battery,
            "agent_plan": {
                "compliance_window": [0, 10],
                "min_soc_floor": "high",  # bad type
                "max_export_kw_override": [1, 2, 3],  # bad type
            },
        }
        _, outputs = engine.step(state, do_nothing_action())
        assert outputs["cost_breakdown"]["compliance_penalty"] == pytest.approx(0.0)
